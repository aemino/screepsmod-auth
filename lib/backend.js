...
function setupRouter (config) {
  ...
  router.get('/api/user/auth-token', authroute.tokenAuth, (req, res) => {
    return tokens.getTokens(req.user)
  })
  router.post('/api/user/auth-token', authroute.tokenAuth, bodyParse, (req, res) => {
    return tokens.addToken(req.user, req.body)
  })
  router.post('/api/user/delete-auth-token', authroute.tokenAuth, bodyParse, (req, res) => {
    return tokens.delToken(req.user, req.body.tokenId)
  })
